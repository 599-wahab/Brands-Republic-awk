...
import {
  ...
  CircleUserRound, Command, Download, FileSpreadsheet, HelpCircle, Home,
  ...
  return (
    <div className="app-shell">
      ...
      <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
        <div className="brand"><div className="brand-mark"><Sparkles size={17} /></div><div><b>Northstar</b><span>Customer operations</span></div><button className="close-mobile" onClick={() => setMobileOpen(false)}><X size={19} /></button></div>
        <div className="workspace"><div className="workspace-avatar">NS</div><div><b>Northstar Commerce</b><span>Operations workspace</span></div><ChevronDown size={15} /></div>
        <nav>
          <p>Workspace</p>
          {nav.map(([label, Icon], i) => <button className={i === 0 ? "active" : ""} key={label as string} onClick={() => act(`${label} module is ready for the next phase`)}><Icon size={17} /><span>{label}</span>{label === "Customers" && <em>8,964</em>}</button>)}
        ...
        <div className="sidebar-bottom">
          ...
          <div className="profile"><div className="avatar">OM</div><div><b>Olivia Martin</b><span>Administrator</span></div><MoreHorizontal size={17} /></div>
        </div>
      </aside>
      ...
      <main>
        ...
        <div className="content">
          <section className="page-heading"><div><p>OVERVIEW</p><h1>Good afternoon, Olivia</h1><span>Here’s what’s happening with your customer operations today.</span></div><div className="heading-actions"><button onClick={() => act("Report exported successfully")}><Download size={16} />Export report</button><button className="primary" onClick={() => act("Import center opened")}><Import size={16} />Import data</button></div></section>
...
function Metric({ label, value, change, note, icon: Icon, tone, negative = false }: any) { return <article className="metric card"><div className={`metric-icon ${tone}`}><Icon size={18}/></div><span>{label}</span><strong>{value}</strong><div className={negative ? "down" : "up"}>{change} <small>{note}</small></div></article>; }
function CardHeader({ title, subtitle, children }: any) { return <div className="card-header"><div><h2>{title}</h2><p>{subtitle}</p></div>{children}</div>; }
function Legend({ color, label, value, percent }: any) { return <div className="legend-row"><span className="dot" style={{ background: color }}/><b>{label}</b><span>{value}</span><em>{percent}</em></div>; }
function Quick({ icon: Icon, label, detail, onClick }: any) { return <button className="quick" onClick={onClick}><div><Icon size={17}/></div><span><b>{label}</b><small>{detail}</small></span><ArrowRight size={15}/></button>; }
